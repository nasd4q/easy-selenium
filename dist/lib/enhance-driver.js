...
class EnhancedDriver extends WebDriver {
    ...
    async waitAndFindByText(text, timeout, trials) {
        ...
        do {
            await new Promise(r => setTimeout(r, timeout));
            found = await this.executeScript(function innermostWithText(text) {
                let v = document.evaluate(`//*[contains(text(), "` + text + `") and not (./*[contains(text(), "` + text + `")])]`, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
